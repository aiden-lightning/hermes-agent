...
def get_outbound_media_allowlist(*, include_defaults: bool = True) -> list[Path]:
    ...
    if include_defaults:
        ...
        try:
            ...
            raw_cfg = yaml.safe_load(cfg_path.read_text(encoding="utf-8")) if cfg_path.exists() else None
            gateway_cfg = raw_cfg.get("gateway", {}) if isinstance(raw_cfg, dict) else {}
            if isinstance(gateway_cfg, dict) and "outbound_media_allowlist" in gateway_cfg:
                configured = gateway_cfg.get("outbound_media_allowlist")
        except Exception:
            configured = None
...
def _strip_media_directives_for_display(text: str) -> str:
    """Remove delivery directives outside markdown code spans."""
    if not text:
        return text
    ...
    for segment_start, segment_end in _non_markdown_code_spans(text, code_spans):
        ...
        media_spans.extend(
            ...
            for match in MEDIA_TAG_CLEANUP_RE.finditer(segment)
        )
...
def _media_delivery_allowed_roots() -> List[Path]:
    ...
    roots.extend(get_outbound_media_allowlist(include_defaults=False))
...
def _strip_media_directives(text: str) -> str:
    ...
    exact.
    """
    return _strip_media_directives_for_display(text)
...
class BasePlatformAdapter(ABC):
    ...
    def _mask_protected_spans(content: str) -> str:
        ...
        for m in re.finditer(r'`[^`\n]+`', content):
            start = m.start()
            if _is_backticked_media_directive(content, start, m.end()):
                continue
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        # Merge platform-aware thread metadata (Telegram DM topic routing) with
        # any per-event metadata (e.g. Feishu inbound mention refs for outbound
        # @ rendering) carried on ``event.metadata``.
        _thread_metadata = dict(getattr(event, "metadata", None) or {})
        _routing_meta = _thread_metadata_for_source(event.source, _reply_anchor_for_event(event))
        if _routing_meta:
            _thread_metadata.update(_routing_meta)
        ...
        try:
            ...
            if response:
                ...
                if text_content and not _tts_caption_delivered:
                    response_metadata = dict(_final_thread_metadata or {})
