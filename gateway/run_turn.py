...
class GatewayTurnMixin:
    ...
    async def _hmwa_prepare_turn(self, event, source, session_entry, session_key, _quick_key, run_generation):
        ...
        _session_enabled_toolsets, _ = self._resolve_turn_toolsets(
            _session_user_config,
            source,
            _platform_config_key(source.platform),
            self._collect_event_sender_ids(event),
        )
        # Session context variables for tools (task-local, concurrency-safe)
        _session_env_tokens = self._set_session_env(
            context, enabled_toolsets=_session_enabled_toolsets,
        )
        ...
        with suppress(Exception):
            _redact_pii = bool((_session_user_config.get("privacy") or {}).get("redact_pii", False))
    ...
    def _resolve_enabled_toolsets_for_source(
        ...
        return self._resolve_effective_enabled_toolsets(
            user_config=user_config, source=source, actor_ids=actor_ids,
            platform_key=platform_key,
        )

    def _resolve_turn_toolsets(
        self, user_config: dict, source: "SessionSource", platform_key: str,
        actor_ids: Optional[set[str]] = None,
    ):
        """``(enabled_toolsets, disabled_toolsets)`` for an agent run on ``source``."""
        from agent.skill_utils import parse_config_string_list
        enabled = self._resolve_enabled_toolsets_for_source(user_config, source, platform_key, actor_ids)
    ...
    async def _run_background_task_inner(
        ...
        try:
            ...
            actor_ids = self._collect_source_sender_ids(source)
            enabled_toolsets, disabled_toolsets = self._resolve_turn_toolsets(
                user_config, source, platform_key, actor_ids,
            )
            ...
            def run_sync():
                agent = AIAgent(
                    ...
                    **{k: getattr(source, k) for k in (
                    ...
                    session_search_include_unowned_user_sessions=session_search_filters.get(
    ...
    def _run_agent_display_settings(
        self, source: SessionSource, actor_ids: Optional[set[str]] = None,
        ...
        return self._RunAgentDisplay(
            ...
            disabled_toolsets=disabled_toolsets, session_search_filters=session_search_filters,
    ...
    async def _run_agent_queued_followup(
        ...
        try:
            ...
            followup_result = await self._run_agent(
                ...
                actor_ids=self._collect_event_sender_ids(pending_event) if pending_event is not None else None,
            )
        except asyncio.CancelledError:
    ...
    async def _run_agent_inner(
        ...
        disp = self._run_agent_display_settings(source, actor_ids)
