...
class GatewayInboundMixin:
    ...
    async def _hm_handle_running_session_message(
        ...
        permission_denial = self._command_permission_denial_for_event(event)
    ...
    async def _hm_dispatch_idle_commands(
        ...
        if not _handled:
            _result = self._check_command_permissions(
                event, raw_command=command, canonical_command=canonical,
            )
