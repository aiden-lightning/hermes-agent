...
def build_session_owner_key(
    ...
    """Owner key used by session-search visibility, distinct from permission identity."""
    if is_shared_multi_user_session(
        source,
        group_sessions_per_user=group_sessions_per_user,
        thread_sessions_per_user=thread_sessions_per_user,
    ):
        return "session_scope:" + build_session_key(
            source,
            group_sessions_per_user=group_sessions_per_user,
            thread_sessions_per_user=thread_sessions_per_user,
        )
    owner = str(source.user_id_alt or source.user_id or "").strip()
