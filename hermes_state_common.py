...
def normalize_user_id_filter(user_id_filter: Any) -> "list[str] | None":
    """Trimmed, non-empty owner ids, or None when no ownership scoping was requested.
    An explicit empty list stays empty (a caller scoped to nobody must match nothing,
    never fall open to every user's sessions)."""
    if user_id_filter is None:
        return None
    return [str(v).strip() for v in user_id_filter if str(v).strip()]


def session_owner_where(
    user_id_filter: Any, include_unowned_user_sessions: bool = False,
) -> "tuple[str, list[str]]":
    """``(clause, params)`` scoping ``sessions s`` to *user_id_filter*, optionally OR'd with legacy
    unowned rows (NULL/empty ``user_id``, minted before ownership was stamped). Fail-closed:
    scoping to nobody yields ``"0"`` (match nothing), so a caller that resolved an empty owner set
    can never read another user's history. ``("", [])`` means no ownership scoping was requested."""
    owners = normalize_user_id_filter(user_id_filter)
    if owners is None and not include_unowned_user_sessions:
        return "", []
    owners = owners or []
    parts: list[str] = []
    params: list[str] = []
    if owners:
        parts.append(f"s.user_id IN ({','.join('?' for _ in owners)})")
        params.extend(owners)
    if include_unowned_user_sessions:
        parts.append("(s.user_id IS NULL OR s.user_id = '')")
    if not parts:
        return "0", []
