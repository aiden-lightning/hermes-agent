...
def init_agent(
    ...
    gateway_session_key: str = None, session_owner_user_id: str = None,
    ...
    # Session-search scoping (gateway multi-user). The owner defaults through the stable alternate
    # id before the raw platform id, so a rename doesn't re-scope an existing session's history.
    agent._session_owner_user_id = session_owner_user_id or user_id_alt or user_id
    # None = unfiltered; an explicitly empty list stays empty (matches nothing) rather than
    # silently widening to "all sources".
    agent._session_search_source_filter = list(session_search_source_filter or []) or None
    agent._session_search_user_id_filter = (
        list(session_search_user_id_filter) if session_search_user_id_filter is not None else None
    )
    agent._session_search_include_unowned_user_sessions = bool(
        session_search_include_unowned_user_sessions)
