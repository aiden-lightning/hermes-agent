...
def _session_search(agent, args: dict, ctx: InlineToolContext) -> Any:
    ...
    return _call_tool(
        ...
        # Gateway multi-user visibility scope, set at init from the platform identity. Absent
        # (CLI/TUI) these are None/False and the search is unscoped.
        source_filter=getattr(agent, "_session_search_source_filter", None),
        user_id_filter=getattr(agent, "_session_search_user_id_filter", None),
        include_unowned_user_sessions=getattr(
            agent, "_session_search_include_unowned_user_sessions", False),
    )
