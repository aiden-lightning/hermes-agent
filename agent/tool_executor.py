...
def _log_tool_call_start(agent, effective_task_id: str, function_name: str, function_args: dict, tool_call_id: str) -> None:
    logger.info(
        ...
        function_name,
        tool_call_id or "",
        _format_tool_observability(function_name, function_args),
    )
...
def _log_tool_call_end(
    ...
    result_chars = len(_multimodal_text_summary(result))
    log_fn = logger.warning if is_error else logger.info
    log_fn(
        "tool call end session=%s task_id=%s tool=%s tool_call_id=%s duration=%.2fs status=%s %s result_chars=%d",
        getattr(agent, "session_id", None) or "none",
        effective_task_id or "",
        function_name,
        tool_call_id or "",
