...
# "run_agent" (not __name__): tool-call tracing must land in the same logger the rest of the agent
# run writes to, so a gateway session's trace reads as one stream after the tool-dispatch split.
logger = logging.getLogger("run_agent")


def _format_tool_observability(function_name: str, function_args: dict, result: Any = None) -> str:
    """Redacted key=value trace fields for one tool call (defined in run_agent)."""
    ra = _ra()
    return ra._format_observability_fields(
        ra._tool_call_observability_fields(function_name, function_args, result))


def _log_tool_call_start(agent, effective_task_id: str, function_name: str, function_args: dict,
                         tool_call_id: str) -> None:
    logger.info(
        "tool call start session=%s task_id=%s tool=%s tool_call_id=%s %s",
        getattr(agent, "session_id", None) or "none", effective_task_id or "", function_name,
        tool_call_id or "", _format_tool_observability(function_name, function_args))


def _log_tool_call_end(agent, effective_task_id: str, function_name: str, function_args: dict,
                       tool_call_id: str, duration: float, result: Any, is_error: bool) -> None:
    log_fn = logger.warning if is_error else logger.info
    log_fn(
        "tool call end session=%s task_id=%s tool=%s tool_call_id=%s duration=%.2fs status=%s %s "
        "result_chars=%d",
        getattr(agent, "session_id", None) or "none", effective_task_id or "", function_name,
        tool_call_id or "", duration, "error" if is_error else "success",
        _format_tool_observability(function_name, function_args, result),
        len(_multimodal_text_summary(result)))
...
def _commit_tool_result(
    ...
    *,
    ...
    if observed:
        ...
        # Structured trace (redacted key=value fields) pairing with the "tool call start" line;
        # success_log_chars is None on the concurrent path, which logs its own end line.
        if is_error or success_log_chars is not None:
            _log_tool_call_end(agent, effective_task_id, function_name, function_args,
                               tool_call_id or "", tool_duration, function_result, is_error)
        if not blocked:
            try:
...
class _ConcurrentBatch:
    ...
    def _dispatch_worker(self, index: int, ref: _ToolCallRef, scope_block, start_gate: _WorkerStartOnce) -> Optional[_ToolOutcome]:
        ...
        start = time.time()
        _log_tool_call_start(agent, ref.task_id, ref.name, ref.args, ref.call_id or "")
        ...
        _log_tool_call_end(agent, ref.task_id, ref.name, ref.args, ref.call_id or "", duration,
                           result, is_error)
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(tool_calls, 1):
        ...
        _log_tool_call_start(agent, effective_task_id, ref.name, ref.args, ref.call_id or "")
