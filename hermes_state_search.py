...
def _search_filter_clauses(
    ...
    user_id_filter: Optional[List[str]] = None, include_unowned_user_sessions: bool = False,
    ...
    filled by out-of-window hits. ``user_id_filter`` scopes hits to those session owners on
    every route (see :func:`session_owner_where`), so a shared-platform search can never
    surface another user's transcript through a fallback path."""
