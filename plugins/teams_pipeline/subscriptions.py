...
def is_managed_subscription(store: TeamsPipelineStore, subscription_payload: dict[str, Any], *, expected_client_state_value: str | None) -> bool:
    ...
    if not (expected_client_state_value and candidate_state):
        return False
    # Constant-time: a secret-dependent early-exit compare leaks clientState prefixes to an
    # attacker who can replay webhook payloads.
    return hmac.compare_digest(candidate_state, expected_client_state_value)
