...
class TestBrowserVisionAnnotate:
    ...
    def test_cdp_screenshot_clip_uses_css_dimensions_without_doubling_dpr(self):
        ...
        from tools.browser_tool_cdp import _build_cdp_screenshot_clip
    ...
    def test_cdp_browser_vision_does_not_call_agent_browser_full(self, tmp_path):
        ...
        with (
            ...
            patch(
                "tools.browser_tool_cdp._get_cdp_screenshot_endpoint",
                return_value="ws://example/devtools/browser/1",
            ),
            patch(
                "tools.browser_tool_cdp._capture_cdp_screenshot",
                side_effect=fake_capture,
            ) as mock_cdp_capture,
            patch("tools.browser_tool_session._run_browser_command") as mock_cmd,
            patch("tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("agent.auxiliary_client.call_llm", return_value=mock_response),
        ):
    ...
    def test_annotate_true_uses_agent_browser_fallback_with_annotate(self):
        ...
        with (
            ...
            patch("tools.browser_tool_cdp._capture_cdp_screenshot") as mock_cdp_capture,
            patch("tools.browser_tool_session._run_browser_command") as mock_cmd,
