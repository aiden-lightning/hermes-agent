...
Four calling shapes:
  ...
  3. READ      — pass session_id only → bounded whole-session dump
...
class TestShapePrecedence:
    ...
    def test_current_root_session_excludes_child_lineage(self):
        ...
        mock_db = MagicMock()
        mock_db.search_messages.return_value = [
            {"id": 1, "session_id": "child_sid", "content": "match", "source": "cli",
    ...
    def test_source_from_resolved_parent_not_fts5_child(self):
        ...
        mock_db.get_anchored_view.return_value = {
            "bookend_start": [],
            "window": [{"id": 42, "role": "user", "content": "hello world"}],
            "bookend_end": [],
            "messages_before": 0,
            "messages_after": 0,
        }
        ...
        assert entry["session_id"] == "child_sid", "should keep anchor-owning session ID for scrolling"
