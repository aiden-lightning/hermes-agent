...
def test_managed_subscription_uses_timing_safe_client_state(monkeypatch, tmp_path):
    from plugins.teams_pipeline import subscriptions

    calls = []
    real_compare = hmac.compare_digest

    def spy_compare(left, right):
        calls.append((left, right))
        return real_compare(left, right)

    monkeypatch.setattr(subscriptions.hmac, "compare_digest", spy_compare)
    store = TeamsPipelineStore(tmp_path / "teams-store.json")

    assert subscriptions.is_managed_subscription(
        store,
        {"id": "remote-sub", "clientState": "expected-state"},
        expected_client_state_value="expected-state",
    ) is True
