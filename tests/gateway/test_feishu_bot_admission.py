...
def test_feishu_load_settings_ignore_at_all(monkeypatch, env_value, extra, expected):
    ...
    assert FeishuAdapter._load_settings(extra=extra).ignore_at_all is expected


def test_feishu_at_all_only_counts_as_self_mention_when_enabled():
    from plugins.platforms.feishu.adapter import FeishuAdapter
