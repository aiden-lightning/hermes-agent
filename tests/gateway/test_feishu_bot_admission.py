...
def test_should_accept_group_message_passes_bot_context_to_group_policy():
    adapter = make_adapter_skeleton(
        bot_open_id="ou_self",
        allow_bots="all",
        require_mention=False,
...
def test_should_accept_group_message_uses_per_group_require_mention():
    from gateway.platforms.feishu import FeishuGroupRule
    ...
    sender = make_sender(sender_type="user", open_id="ou_human")
    message = make_message(chat_id="oc_free", chat_type="group")

    assert adapter._should_accept_group_message(message, sender.sender_id, "oc_free") is True
...
def test_handle_message_event_data_keeps_admitted_bot_group_message():
    import asyncio
    ...
    async def _fake_cache_group_message(**_kwargs):
        return ("", None, [], [], [], {})

    async def _fake_process_inbound_message(**kwargs):
        captured.append(kwargs)

    adapter._cache_group_message = _fake_cache_group_message
    adapter._process_inbound_message = _fake_process_inbound_message

    message = make_message(message_id="om_bot_group", chat_type="group")
    ...
    assert captured[0]["message_id"] == "om_bot_group"


def test_handle_message_event_data_keeps_per_group_unmentioned_message():
    import asyncio
    ...
    message = make_message(message_id="om_free_group", chat_id="oc_free", chat_type="group")
    message.message_type = "image"
    sender = make_sender(sender_type="user", open_id="ou_human")
    data = SimpleNamespace(event=SimpleNamespace(sender=sender, message=message))
    ...
    assert captured[0]["message_id"] == "om_free_group"
