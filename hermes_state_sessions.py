...
from hermes_state_common import (
    ...
    _SQL_IN_CHUNK, _id_chunks, _placeholders as _session_ids_placeholders, session_owner_where,
)
...
def _session_filter_where(
    *, exclude_children: bool = False, source: str = None, sources: List[str] = None,
    ...
    user_id_filter: List[str] = None, include_unowned_user_sessions: bool = False,
    ...
    (``_LISTABLE_CHILD_SQL``). ``user_id_filter`` scopes rows to those owners, optionally OR'd with
    legacy unowned (NULL/empty ``user_id``) rows. Clause order is part of the SQL text contract."""
    where: List[str] = []
    params: List[Any] = []
    if exclude_children:
    ...
    owner_clause, owner_params = session_owner_where(user_id_filter, include_unowned_user_sessions)
    if owner_clause:
...
""" + _UPSERT_KEEP_EXISTING_SQL,
            ...
            if user_id:
                # user_id is absent from _UPSERT_KEEP_EXISTING_SQL, so the upsert never
                # stamps ownership onto a row an earlier writer minted unowned. Fill that
                # NULL here, and let a `session_scope:` key overwrite an existing owner:
                # a shared group/thread session belongs to the namespaced scope, not to
                # whichever participant happened to create the row first.
                conn.execute(
                    """UPDATE sessions
                       SET user_id = CASE
    ...
    def list_recent_sessions_bounded(
        ...
        *,
        ...
        # Gateway multi-user scoping. Applied in the candidate preselect (not post-hoc on the
        # result) so `limit` counts visible rows: filtering after the LIMIT would let another
        # user's sessions consume the budget and silently shrink the caller's own browse.
        if source_filter is not None:
            if not source_filter:
                candidate_clauses.append("0")  # scoped to no source → match nothing
            else:
                placeholders = ",".join("?" for _ in source_filter)
                candidate_clauses.append(f"s.source IN ({placeholders})")
                candidate_params.extend(source_filter)
        owner_clause, owner_params = session_owner_where(
            user_id_filter, include_unowned_user_sessions)
        if owner_clause:
            candidate_clauses.append(owner_clause)
            candidate_params.extend(owner_params)
        # The candidate preselect scopes the tip; the final projection joins the lineage ROOT as
        # ``s`` and reports its title/started_at, so the root is scoped too — otherwise a
        # compression lineage whose root belongs to another user would surface through its tip.
        root_scope_clauses: List[str] = []
        root_scope_params: List[Any] = []
        if source_filter is not None:
            if not source_filter:
                root_scope_clauses.append("0")
            else:
                root_scope_clauses.append(
                    f"s.source IN ({','.join('?' for _ in source_filter)})")
                root_scope_params.extend(source_filter)
        if owner_clause:
            root_scope_clauses.append(owner_clause)
            root_scope_params.extend(owner_params)
        root_scope_sql = " AND ".join(root_scope_clauses) if root_scope_clauses else "1"
    ...
    def list_sessions_rich(
        ...
        ``user_id_filter`` scopes rows to those owners (see :func:`session_owner_where`)."""
