...
_TTS_PROVIDER_CHOICES = [
    ...
    ("moss", "MOSS-TTS-Nano (local ONNX CPU, bundled reference voice)")]
...
def _tts_moss_step() -> str:
    """Select the bundled MOSS runtime only when its checkout and deps are ready."""
    try:
        from tools.tts_tool import _check_moss_available
        available = _check_moss_available()
    except Exception:
        available = False
    if available:
        _setup.print_success("MOSS-TTS-Nano integration is available")
        return "moss"
    print()
    _setup.print_warning("MOSS-TTS-Nano is not ready in this checkout.")
    _setup.print_info("Initialize the bundled moss_tts_nano_repo and install its local dependencies, then re-run setup.")
    _setup.print_info("Typical dependencies: sentencepiece and torchaudio")
    return "edge"
