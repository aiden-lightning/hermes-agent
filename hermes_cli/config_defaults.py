...
DEFAULT_CONFIG = {
    ...
    # NeuTTS/KittenTTS/MOSS 2000).
    "tts": {
        # "edge" (free) | "elevenlabs" (premium) | "openai" | "xai" | "minimax" | "mistral" |
        # "gemini" | "deepinfra" | "neutts" (local) | "kittentts" (local) | "moss" (local) |
        # "piper" (local)
        "provider": "edge",
        "edge": {
        ...
        "moss": {
            "model_dir": "",     # empty = runtime default inside the bundled repo
            "prompt_audio": "",  # path to reference voice audio (empty = bundled zh_1.wav)
            "cpu_threads": 4,
            "sample_mode": "fixed",  # greedy, fixed, or full
        },
        "piper": {
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # Local paths that text-extracted outbound MEDIA: directives and auto-detected local file
        # attachments may upload to messaging platforms. Explicit tool attachments bypass this leak
        # guard. Paths support ~ and env vars; ~/.hermes resolves to the active Hermes profile home.
