...
def _safe_dependency_check_argv(check_cmd: object) -> list[str] | None:
    """Return argv for a non-shell dependency probe, or None for unsafe input."""
    if isinstance(check_cmd, str):
        if "$" in check_cmd or any(char in check_cmd for char in _UNSAFE_CHECK_CHARS):
            return None
        try:
    ...
    return None if executable in _SHELL_EXECUTABLES else argv
    ...
    for dep in meta.get("external_dependencies", []):
        ...
        if check_cmd:
            ...
            if check_argv is None:
                print(f"\n  ⚠ Unsafe dependency check for '{dep.get('name', '')}' skipped.")
                if install_cmd:
                    print(f"    Install manually: {install_cmd}")
                continue
            try:
                subprocess.run(check_argv, shell=False, check=True, capture_output=True, timeout=5)
            except Exception:
                if install_cmd:
