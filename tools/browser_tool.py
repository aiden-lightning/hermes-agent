...
def _allow_private_urls() -> bool:
    """Return whether private/internal URLs are allowed.

    Checks (in priority order):
    1. ``security.allow_private_urls`` in config.yaml
    2. ``browser.allow_private_urls`` in config.yaml (legacy fallback)

    Result cached for process lifetime.  Even when this returns True,
    cloud metadata endpoints (IMDS) are still always blocked by the
    ``is_always_blocked_url`` floor.
    """
