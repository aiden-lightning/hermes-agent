...
def _capture_vision_screenshot(effective_task_id: str, annotate: bool, screenshot_path: Path, lp_prerouted: bool):
    ...
    if lp_prerouted and screenshot_path.exists():
        ...
        cdp_endpoint = "" if annotate else _cdp._get_cdp_screenshot_endpoint(effective_task_id)
        if cdp_endpoint:
            result = _cdp._capture_cdp_screenshot(cdp_endpoint, screenshot_path)
        else:
            ...
            result = _session._run_browser_command(
                effective_task_id, "screenshot", screenshot_args,
...
from tools import browser_tool_connect as _connect  # noqa: F401 - registers browser_connect
