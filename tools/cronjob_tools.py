...
from tools import cronjob_access
...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    owner = a["_gateway_owner"]
    is_admin = a["_gateway_is_admin"]
    refs = _clean_str_list(context_from)
    permission_error = cronjob_access.context_reference_permission_error(refs, owner, is_admin)
    if permission_error:
        return tool_error(permission_error, success=False)
    resolved_toolsets, toolset_error = cronjob_access.resolve_job_toolsets(
        a["enabled_toolsets"], owner, is_admin,
    )
    if toolset_error:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            enabled_toolsets=resolved_toolsets, workdir=_normalize_optional_job_value(a["workdir"]),
...
def _action_list(a: Dict[str, Any]) -> str:
    raw_jobs = cronjob_access.filter_visible_jobs(
        list_jobs(include_disabled=a["include_disabled"]),
        a["_gateway_owner"],
        a["_gateway_is_admin"],
    )
...
def _update_context_from(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    ...
    if refs:
        ...
        if ref_error:
            return ref_error
        permission_error = cronjob_access.context_reference_permission_error(
            refs, a["_gateway_owner"], a["_gateway_is_admin"],
        )
        if permission_error:
...
def _update_run_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    ...
    if a["enabled_toolsets"] is not None:
        resolved_toolsets, toolset_error = cronjob_access.resolve_job_toolsets(
            a["enabled_toolsets"], a["_gateway_owner"], a["_gateway_is_admin"],
        )
        if toolset_error:
            return toolset_error
...
def cronjob(
    ...
    try:
        ...
        owner = cronjob_access.gateway_owner_scope()
        is_admin = cronjob_access.caller_is_admin(owner)
        a["_gateway_owner"] = owner
        a["_gateway_is_admin"] = is_admin
        if normalized == "resnap":
            if owner and bool(a["all"]) and not is_admin:
                return tool_error(
                    "Permission denied for bulk cron resnap; only a platform admin may modify all jobs.",
                    success=False,
                )
            if owner and a["job_id"]:
                job, error = _resolve_job_or_error(a["job_id"])
                if error is not None:
                    return error
                if not cronjob_access.can_access_job(job, owner, is_admin):
                    return tool_error(cronjob_access.permission_denial(a["job_id"]), success=False)
            return _action_resnap(a)
        ...
        if error is not None:
            return error
        if not cronjob_access.can_access_job(job, owner, is_admin):
            return tool_error(cronjob_access.permission_denial(job_id), success=False)
