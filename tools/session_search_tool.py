...
mode parameter):
  ...
  3. READ — pass ``session_id`` without ``around_message_id``. Returns a
     bounded whole-session dump, including cross-profile @session links.
...
def _list_recent_sessions(
    ...
    try:
        ...
        if source_filter and len(source_filter) == 1:
            list_kwargs["source"] = source_filter[0]
        if user_id_filter is not None:
            list_kwargs["user_id_filter"] = user_id_filter
            list_kwargs["include_unowned_user_sessions"] = include_unowned_user_sessions
...
registry.register(
    ...
    handler=lambda args, **kw: session_search(
        ...
        source_filter=kw.get("source_filter"),
        user_id_filter=kw.get("user_id_filter"),
