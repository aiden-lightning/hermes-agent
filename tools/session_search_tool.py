...
class _Scope:
    """Per-caller session visibility scope (gateway multi-user isolation).

    A CLI caller gets the unrestricted scope (every field ``None``/False). A gateway caller is
    scoped to its own platform + owner ids, so one platform user cannot read another's transcripts
    by guessing a session id. Every shape (discover/scroll/read/browse) is filtered — a scoped miss
    is reported as "not found", never as "forbidden", so the tool doesn't confirm id existence.
    """

    __slots__ = ("source_filter", "user_id_filter", "include_unowned")

    def __init__(self, source_filter=None, user_id_filter=None, include_unowned: bool = False):
        # An explicitly empty list stays empty (matches nothing); None means unfiltered.
        self.source_filter = (
            [str(v).strip() for v in source_filter if str(v).strip()]
            if source_filter is not None else None)
        self.user_id_filter = (
            [str(v).strip() for v in user_id_filter if str(v).strip()]
            if user_id_filter is not None else None)
        self.include_unowned = bool(include_unowned)

    @property
    def restricted(self) -> bool:
        return (self.source_filter is not None or self.user_id_filter is not None
                or self.include_unowned)

    def allows(self, meta: Dict[str, Any]) -> bool:
        """Whether a session's metadata is visible in this scope."""
        if not self.restricted:
            return True
        if not meta:
            return False
        if self.source_filter is not None:
            if str(meta.get("source") or "").strip() not in self.source_filter:
                return False
        if self.user_id_filter is None and not self.include_unowned:
            return True
        owner = str(meta.get("user_id") or "").strip()
        if self.user_id_filter and owner in self.user_id_filter:
            return True
        # Sessions predating per-user attribution carry no owner; the gateway opts into them
        # explicitly rather than having them silently visible to every user.
        return self.include_unowned and not owner
...
def _title_match_result(db, query: str, current_lineage_root: Optional[str],
                        scope: "_Scope" = _UNSCOPED) -> Optional[Dict[str, Any]]:
    ...
    if session_meta.get("source") in _HIDDEN_SESSION_SOURCES:
        return None
    # Title resolution bypasses the FTS query (where scoping is applied in SQL), so this is the
    # one discovery path that must re-check visibility in Python.
    if not scope.allows(session_meta):
        return None
...
def _discover(db, query: str, role_filter: Optional[List[str]], limit: int, sort: Optional[str],
    ...
    title_result = _title_match_result(db, query, current_lineage_root, scope)
    ...
    raw_results, err = _loud(lambda: db.search_messages(
        ...
        source_filter=scope.source_filter, user_id_filter=scope.user_id_filter,
        include_unowned_user_sessions=scope.include_unowned), "FTS5 search failed: %s", "Search failed")
    if err:
        return err
...
def _read_session(db, session_id: str, head: int = 20, tail: int = 10, link_profile: str = None,
    ...
    if not meta or not scope.allows(meta):
...
def _read_scoped(db, sid: str, profile: Optional[str], scope: "_Scope" = _UNSCOPED) -> str:
    ...
    """
    result = _read_session(db, sid, link_profile=profile, scope=scope)
...
def _list_recent_sessions(db, limit: int, current_session_id: str = None, link_profile: str = None,
    ...
    def _browse():
        ...
        sessions = bounded_list(
            ...
            **({"source_filter": scope.source_filter} if scope.source_filter is not None else {}),
            **({"user_id_filter": scope.user_id_filter,
                "include_unowned_user_sessions": scope.include_unowned}
               if scope.user_id_filter is not None or scope.include_unowned else {}))
...
def _scroll(db, session_id: str, around_message_id: int, window: int = 5,
            current_session_id: str = None, scope: "_Scope" = _UNSCOPED) -> str:
    ...
    if not session_meta or not scope.allows(session_meta):
        return tool_error(f"session_id not found: {session_id}", success=False)
    ...
    if not messages and owning and owning != session_id:
        ...
        if rebind_view and rebind_view.get("window"):
            # The rebind target is a DIFFERENT session, so it needs its own visibility check —
            # otherwise a visible parent id plus a descendant's message id would serve a window
            # from a session the caller may not read.
            rebound_meta = _get_session_meta(db, owning) or session_meta
            if not scope.allows(rebound_meta):
...
def _dispatch(query, role_filter, limit, db, current_session_id, session_id,
    ...
    if isinstance(session_id, str) and session_id.strip():
        if around_message_id is not None:
            return _scroll(db, session_id.strip(), around_message_id, window, current_session_id,
                           scope=scope)
        return _read_scoped(db, session_id.strip(), profile, scope=scope)
    ...
    return _discover(
        ...
        exclude_session_ids=_normalize_exclude_session_ids(exclude_session_ids), scope=scope)
...
def session_search(query: str = "", role_filter: str = None, limit: int = 3, db=None,
                   ...
                   include_unowned_user_sessions: bool = False) -> str:
    ...
    ``source_filter`` / ``user_id_filter`` / ``include_unowned_user_sessions`` scope visibility for
    gateway multi-user callers (see :class:`_Scope`); omitted, the caller sees every session."""
    ...
    try:
        return _dispatch(query, role_filter, limit, db, current_session_id, session_id,
                         ...
                         scope=_Scope(source_filter, user_id_filter, include_unowned_user_sessions))
    finally:
...
registry.register(
    ...
    handler=lambda args, **kw: session_search(
        ...
        # Visibility scope comes from the CALLER (kw), never from model-supplied args — a model
        # that could set these would scope itself out of isolation.
        source_filter=kw.get("source_filter"), user_id_filter=kw.get("user_id_filter"),
        include_unowned_user_sessions=bool(kw.get("include_unowned_user_sessions")),
