...
def _await_gateway_decision(
    ...
    *,
    ...
    observed_command = payload["command"] if log_command is None else log_command
    observed_description = payload["description"] if log_description is None else log_description
    ...
    outcome = "cancelled" if cancelled else ("timeout" if state == "timeout" else (choice or "timeout"))
    log_fn = logger.warning if outcome == "timeout" else logger.info
    log_fn(
