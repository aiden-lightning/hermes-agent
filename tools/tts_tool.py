...
from tools.tts_tool_local import (
    _check_moss_available, _default_moss_ref_audio, _generate_kittentts, _generate_moss_tts,
    _generate_neutts, _generate_piper_tts, _import_moss_onnx_runtime, _moss_repo_root)
...
def _has_ffmpeg() -> bool:
    """Whether ffmpeg is on PATH. Voice-bubble delivery for WAV-native local engines depends on it."""
...
_FFMPEG_OPUS_PROVIDERS = frozenset({"edge", "neutts", "minimax", "xai", "kittentts", "piper", "moss"})
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "moss": (lambda: _moss_runtime_importable(), "MOSS-TTS-Nano (local ONNX CPU)", "_generate_moss_tts",
             "MOSS provider selected but MOSS-TTS-Nano dependencies are not installed. "
             "Run `source venv/bin/activate && pip install sentencepiece torchaudio`.")}


def _moss_runtime_importable() -> bool:
    """Whether the vendored MOSS runtime loads. Unlike the pip-packaged engines it raises
    FileNotFoundError/ImportError depending on what is missing, so everything is caught."""
    try:
...
def _finalize_voice_delivery(
    ...
    elif provider == "moss" and file_str.endswith(".ogg"):
        # MOSS is WAV-native and needs ffmpeg to produce real Ogg/Opus. Without ffmpeg its .ogg is
        # a renamed WAV payload, which must not be offered as a voice bubble.
        return file_str, _has_ffmpeg()
    else:
...
_BUILTIN_REQUIREMENTS: Dict[str, Callable[[], bool]] = {
    ...
    "moss": lambda: _check_moss_available()}
