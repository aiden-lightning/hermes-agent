...
def _get_cdp_screenshot_endpoint(task_id: str) -> str:
    """Return the CDP WebSocket endpoint used for direct screenshot capture."""
    _bt = _origin()
    ...
    return _resolve_cdp_override(cdp_url) if cdp_url else ""


def _build_cdp_screenshot_clip(metrics: Dict[str, Any]) -> Dict[str, float]:
    """Build a CSS-pixel clip without applying devicePixelRatio a second time."""
    ...
    return {
        "x": 0,
        "y": 0,
        "width": max(positive(metrics.get("scrollWidth")), positive(metrics.get("innerWidth")), 1),
        "height": max(positive(metrics.get("scrollHeight")), positive(metrics.get("innerHeight")), 1),
        "scale": 1,
    }


def _select_cdp_page_target(targets: List[Dict[str, Any]]) -> str:
    pages = [target for target in targets if target.get("type") == "page" and target.get("targetId")]
    if not pages:
    ...
    return str(selected["targetId"])


async def _capture_cdp_screenshot_async(
    endpoint: str, screenshot_path: Path, timeout: float,
) -> Dict[str, Any]:
    ...
    metrics_result = await _cdp_call(
        ...
        "Runtime.evaluate",
        {
            "expression": (
                ...
                "scrollWidth:Math.max(document.documentElement.scrollWidth||0,"
                "document.body?document.body.scrollWidth||0:0,window.innerWidth||0),"
                "scrollHeight:Math.max(document.documentElement.scrollHeight||0,"
                "document.body?document.body.scrollHeight||0:0,window.innerHeight||0)})"
            ),
            "returnByValue": True,
        },
        target_id,
    ...
    screenshot_result = await _cdp_call(
        ...
        {"format": "png", "fromSurface": True, "captureBeyondViewport": True, "clip": clip},
        target_id,
        timeout,
    )
    ...
    return {"success": True, "path": str(screenshot_path), "clip": clip, "target_id": target_id}
...
def _capture_cdp_screenshot(endpoint: str, screenshot_path: Path) -> Dict[str, Any]:
    ...
    _bt = _origin()
    timeout = max(float(_bt._safe_command_timeout()), 5.0)
